...
func filterWorker[T any](filter fn.Filter[T], in <-chan T, out ChanPush[T]) {
	defer close(out)
	for t := range in {
		if filter(t) {
			out <- t
...
func FilterWithError[T any](size int, filter fn.FilterWithError[T], in <-chan T) (ChanPull[T], ChanPull[error]) {
...
func FilterWithErrorSink[T any](size int, filter fn.FilterWithError[T], sink fn.Sink[error], in <-chan T) ChanPull[T] {
	out := make(chan T, size)
